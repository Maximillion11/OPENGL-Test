...
int main()
{
    ...
    GLFWwindow* window = glfwCreateWindow(Utils::SCR_WIDTH, Utils::SCR_HEIGHT, "LearnOpenGL", NULL, NULL);
    if (window == NULL)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        Utils::deltaTime = currentFrame - lastFrameTime;
        lastFrameTime = currentFrame;

        Input::CheckKeys(window, &camera);
...
{
    Input::GetMouse(&camera, xposIn, yposIn);
}

void scroll_callback(GLFWwindow* window, double xOffset, double yOffset)
{
    Input::GetScrollWheel(&camera, xOffset, yOffset);
}
